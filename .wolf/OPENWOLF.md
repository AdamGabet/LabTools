...
## OpenCode periodic nudge (every 3 tools)

In OpenCode, `.opencode/plugins/openwolf.mjs` increments a tool counter and rewrites **`.wolf/.periodic_checklist.md`** every third tool call. Read that file when it updates — it reminds you to sync todos, append semantic lines to `memory.md`, and re-check `cerebrum.md` before codegen. Hooks track footprint; you still supply the *why*.
