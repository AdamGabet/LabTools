...
BODY_SYSTEMS = os.getenv('BODY_SYSTEMS', _LAB_PATHS.get('BODY_SYSTEMS', ''))
...
def get_temp_desc_path():
    """Get current temp path, or None if no session initialized."""
    return _TEMP_DESC_PATH
...
def _load_temp_description_json() -> dict:
	"""Load temp body systems description. Returns empty if no session or file."""
